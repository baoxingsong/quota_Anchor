...
int pro(int argc, char **argv) {
    ...
    double calculateIndelDistance = 3;
    double GAP_OPEN_PENALTY = -0.03;
    double GAP_EXTENSION_PENALTY = -0.01;
    int MAX_DIST_BETWEEN_MATCHES = 25;  // between maize and sorghum set it as 25*3000
    int refMaximumTimes = 1;
    int queryMaximumTimes = 2;
    ...
    usage << "Usage: " << PROGRAMNAME
          ...
          " -d   DOUBLE  calculate IndelDistance (default: " << calculateIndelDistance << ")" << std::endl <<
          " -O   DOUBLE  chain open gap penalty (default: " << GAP_OPEN_PENALTY << ")" << std::endl <<
          " -E   DOUBLE  chain extend gap penalty (default: " << GAP_EXTENSION_PENALTY << ")" << std::endl <<
          " -I   DOUBLE  minimum chain score (default: " << MIN_ALIGNMENT_SCORE << ")" << std::endl <<
          " -D   INT     maximum gap size for chain (default: " << MAX_DIST_BETWEEN_MATCHES << ")" << std::endl <<
          " -W   INT     tandem duplication threshold, if you set -m  1 (default: " << OVER_LAP_WINDOW << ")" << std::endl <<
          " -m   STRING  delete tandem ortholog_pair (default: " << delete_tandem << "), " <<
          "0 : delete tandem;" <<  "1 or other integer : retain tandem" << std::endl;
    ...
    if (inputParser.cmdOptionExists("-i") && inputParser.cmdOptionExists("-o")) {
        ...
        while (std::getline(infile, line)) {
            ...
            std::string queryGene = elems[0];
            std::string queryChr = elems[1];
            int queryId = std::stoi(elems[2]);
            int32_t queryStart = stoi(elems[3]);
            int32_t queryEnd = stoi(elems[4]);
            std::string queryStrand = elems[5];
            std::string refGene = elems[6];
            std::string refChr = elems[7];
            int refId = std::stoi(elems[8]);
            int32_t refStart = stoi(elems[9]);
            int32_t refEnd = stoi(elems[10]);
            std::string refStrand = elems[11];
            double alignmentScore = stod(elems[12]) / 100;
...
//            std::cout << "1615line" << orthologPair.getQueryOrder() << "\t"
//            << alignmentMatchsMapT[alignmentMatchsMapT.size()-1].getQueryOrder()
//            << std::endl;
/*
...
//            assert(queryIndexMap[pairedSimilarFragments[idx].getQueryChr()][idi].getQueryId() == idi);
        ...
        if (delete_tandem !=0 ) {
            // the function is different from raw function created by song.
            // sort by querychr refchr querystart and identity/100,respectively.
            orthlogPairSortbyQuery(alignmentMatchsMapT);
            // filter tandem gene by threshold five. This thought comes from MCScan and MCScanX.
            ...
            // match_bin1[0] has a maximum identity value due to orthologPairSortQuery function.
            ...
            orthlogPairSortbyReference(alignmentMatchsMapT);
...
/*       if ((it2->getRefId() != prev_pair2->getRefId() || std::abs(it2->getQueryId() - prev_pair2->getQueryId()) > OVER_LAP_WINDOW)
           && (it2->getRefChr() == prev_pair2->getRefChr() && it2->getQueryChr() == prev_pair2->getQueryChr())){
        alignmentMatchsMapT_cpy2.push_back(*prev_pair2);
    } else if((it2->getRefId() == prev_pair2->getRefId() && std::abs(it2->getQueryId() - prev_pair2->getQueryId()) <= OVER_LAP_WINDOW)
              && (it2->getRefChr() == prev_pair2->getRefChr() && it2->getQueryChr() == prev_pair2->getQueryChr())){
        match_bin2.push_back(*prev_pair2);
    }*/

            // match_bin2[0] has a maximum identity value due to orthlogPairSortbyReference.
...
//                    if (!match_bin2.empty()) {
                        ...
                        match_bin2.clear();
//                    }
//                } else if ((it2->getRefId() == prev_pair2->getRefId() &&
//                            std::abs(it2->getQueryId() - prev_pair2->getQueryId()) <= OVER_LAP_WINDOW)) {
//                    match_bin2.push_back(*it2);
        ...
        // using refIndexMap to restrict max distance between match and match in the geneSyntenic.cpp file.
...
//        std::cout << "t2 - t1" << "\t" << t2 - t1 << std::endl;
        longestPathQuotaGene(alignmentMatchsMapT, alignmentMatchsMap, refIndexMap, queryIndexMap,
                             ...
                             refMaximumTimes, queryMaximumTimes, calculateIndelDistance, block_score);
